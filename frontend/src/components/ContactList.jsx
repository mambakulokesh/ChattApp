...
const ContactList = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get("http://38.77.155.139:8001/user/get-all-users/", {
                ...
                const users = response.data
                    ...
                    .map((apiUser) => ({
                        ...
                        Messages: ["Hello", "Hi", "Can you order something for me", "What do you want"],
                    }));
    ...
    const startVideoCall = () => {
        if (!selectedContact) return alert("Please select a contact to start a video call.");
        setShowVideo(true);
    ...
    return (
        <div className="h-full w-full bg-gray-900 text-white flex flex-col">
            {/* Header */}
            <div className="bg-gray-800 border-b border-gray-700 p-3 flex items-center justify-between shrink-0">
                <div className="flex items-center space-x-2 min-w-0">
                    <img
                        src={user?.avatar === null ? "https://i.pinimg.com/236x/00/80/ee/0080eeaeaa2f2fba77af3e1efeade565.jpg" : user?.avatar}
                        alt={user?.username}
                    ...
                    <div className="flex flex-col leading-tight min-w-0">
                        <h3 className="text-sm font-semibold truncate">{userList?.username}</h3>
                        <p className="text-xs text-gray-400">{userList?.is_active ? "Online" : "Offline"}</p>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto">
                {filteredContacts.length > 0 ? (
                    filteredContacts.map((contact, index) => (
                        ...
                        >
                            <img
                                src={contact.avatar === null ? "https://i.pinimg.com/236x/00/80/ee/0080eeaeaa2f2fba77af3e1efeade565.jpg" : contact.avatar}
                    ...
                    <p className="text-gray-400 text-center mt-4 text-sm">
                        {contactList.length === 0 ? "Loading or no contacts available." : "No contacts found."}
                    </p>
                )}
            </div>
